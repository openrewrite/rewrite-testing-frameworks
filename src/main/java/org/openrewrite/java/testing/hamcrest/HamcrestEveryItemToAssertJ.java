...
public class HamcrestEveryItemToAssertJ extends Recipe {
    @Getter
    final String displayName = "Migrate Hamcrest `everyItem` to AssertJ";

    @Getter
    final String description = "Migrate Hamcrest `everyItem` to AssertJ `allSatisfy` or `hasOnlyElementsOfType`.";

    private static final MethodMatcher ASSERT_THAT_MATCHER = new MethodMatcher("org.hamcrest.MatcherAssert assertThat(..)");
    private static final MethodMatcher EVERY_ITEM_MATCHER = new MethodMatcher("org.hamcrest.*Matchers everyItem(..)");
    private static final MethodMatcher INSTANCE_OF_MATCHER = new MethodMatcher("org.hamcrest.*Matchers instanceOf(..)");

    @Override
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return Preconditions.check(new UsesMethod<>(EVERY_ITEM_MATCHER), new JavaIsoVisitor<ExecutionContext>() {
            @Override
            ...
            private J.MethodInvocation handleInstanceOf(J.MethodInvocation mi, Expression actual, @Nullable Expression reason,
                ...
                JavaTemplate template = JavaTemplate.builder(
                                "assertThat(#{any()})" + reasonTemplate + ".hasOnlyElementsOfType(#{any(java.lang.Class)})")
                        .contextSensitive()
            ...
            private J.MethodInvocation handleGeneralMatcher(J.MethodInvocation mi, Expression actual, @Nullable Expression reason,
                ...
                String reasonTemplate = reason != null ? ".as(#{any(String)})" : "";
                JavaTemplate template = JavaTemplate.builder(
                                "assertThat(#{any()})" + reasonTemplate + ".allSatisfy(arg -> assertThat(arg, #{any()}))")
                        .contextSensitive()
            ...
            private void removeImports() {
                maybeRemoveImport("org.hamcrest.Matchers.everyItem");
                maybeRemoveImport("org.hamcrest.CoreMatchers.everyItem");
